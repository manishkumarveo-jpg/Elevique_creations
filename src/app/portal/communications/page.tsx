...
import { getConversationsForClient, getMessagesForProject } from '@/dashboard/lib/queries/messages'
...
export default async function PortalCommunicationsPage({ searchParams }: Props) {
  const { project: activeProjectId } = await searchParams
  const supabase = await createServerClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return null

  let conversations: Awaited<ReturnType<typeof getConversationsForClient>> = []
  let messages: Awaited<ReturnType<typeof getMessagesForProject>> = []
  let projectName = ''
  let dbError = false

  try {
    conversations = await getConversationsForClient(user.id)
    if (activeProjectId) {
        ...
        <div className="p-msg-sidebar">
          ...
          <div className="p-msg-list">
            {conversations.length === 0 ? (
              <p style={{ padding: '1rem', fontSize: '0.73rem', color: 'var(--p-t3)' }}>No active projects.</p>
            ) : (
              conversations.map(conv => (
                <Link
                  key={conv.project_id}
                  href={`/portal/communications?project=${conv.project_id}`}
